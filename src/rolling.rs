...
//! Logs will be written to this file for the duration of the period
//! and will automatically roll over
...
//!
//! - [minutely]: A new log file in the format of
//! `some_directory/log_file_name_prefix.yyyy-MM-dd-HH-mm`
//! will be created minutely (once per minute)
//! - [hourly]: A new log file in the format of `some_directory/log_file_name_prefix.yyyy-MM-dd-HH`
//! will be created hourly
//! - [daily]: A new log file in the format of `some_directory/log_file_name_prefix.yyyy-MM-dd`
//! will be created daily
//! - [never]: This will result in log file located at `some_directory/log_file_name`
//!
//! [minutely]: minutely
//! [hourly]: hourly
//! [daily]: daily
...
impl RollingFileAppender {
    ...
    ///
    /// Alternatively,
    /// a `RollingFileAppender` can be constructed using one of the following helpers:
    ///
    /// - [minutely],
    /// - [hourly],
    /// - [daily],
    ...
    /// # fn docs() {
    /// use tracing_appender::rolling::{RollingFileAppender, Rotation};
    /// let file_appender = RollingFileAppender::new(Rotation::HOURLY, "/some/directory", "prefix");
    /// # }
    /// ```
    pub fn new(
        rotation: Rotation,
...
/// let rotation = Rotation::MINUTELY;
/// # }
/// ```
///
/// ### Hourly Rotation
...
impl Inner {
    fn new(
        ...
        let log_prefix_raw = file_name_prefix.as_ref().to_str().unwrap();
        let log_filename_prefix = if log_prefix_raw.ends_with(".log") {
            log_prefix_raw.replace(".log", "")
        } else {
            log_prefix_raw.to_string()
        };
        let filename = rotation.join_date(&log_filename_prefix, &now) + ".log";
...
mod test {
    ...
    #[test]
    #[should_panic(expected = "internal error: Rotation::NEVER is impossible to round.")]
